...
describe("runAccountIngest", () => {
  ...
  it("新帖会直传 COS 并回调 instar，再写入去重记录", async () => {
    const repo = createRepo();
    const cosKeys: string[] = [];
    const instarPostIds: string[] = [];
    ...
    await runAccountIngest({
      ...
      media: {
        ...
        instarClient: {
          async notifyPostIngested(payload) {
            instarPostIds.push(payload.postId);
          },
        },
      },
      now: () => new Date("2026-07-03T10:00:00Z"),
    ...
    expect(instarPostIds).toEqual(["v-1"]);
