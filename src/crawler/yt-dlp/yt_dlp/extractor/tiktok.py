...
class TikTokBaseIE(InfoExtractor):
    ...
    def _extract_author_info(self, author_data, stats_data=None):
        """Extract normalized author/user profile info from various TikTok data formats.

        Handles three data source formats:
        - APP API: author_data = aweme_detail, author fields are snake_case under 'author'
        - Web API: author_data = aweme_detail, author fields are camelCase under 'authorInfo'/'author'
        - User page: author_data = user_detail['user'] dict, stats_data = stats or statsV2 dict

        Returns a dict with standardized field names. None values are filtered out.
        """
        # Step 1: locate the author object from APP API or Web API data
        author = traverse_obj(author_data, ('author', {dict})) or \
            traverse_obj(author_data, ('authorInfo', {dict})) or \
            (author_data if isinstance(author_data, dict) and 'uniqueId' in author_data else {})

        info = traverse_obj(author, {
            'uploader': ('unique_id', {str}),
            'uploader_id': ('uid', {str_or_none}),
            'channel': ('nickname', {str}),
            'channel_id': ('sec_uid', {str}),
            'signature': ('signature', {str}),
            'avatar_thumb': ('avatar_thumb', 'url_list', -1, {url_or_none}),
            'avatar_medium': ('avatar_medium', 'url_list', -1, {url_or_none}),
            'avatar_larger': ('avatar_larger', 'url_list', -1, {url_or_none}),
        }) or {}

        # Merge camelCase fields from Web API / user page format
        info.update(traverse_obj(author, {
            'uploader': ('uniqueId', {str}),
            'uploader_id': ('id', {str_or_none}),
            'channel_id': ('secUid', {str}),
            'channel_is_verified': ('verified', {lambda x: bool(x)}),
            'avatar_thumb': ('avatarThumb', {url_or_none}),
            'avatar_medium': ('avatarMedium', {url_or_none}),
            'avatar_larger': ('avatarLarger', {url_or_none}),
        }) or {})

        # Numeric stats: try snake_case (APP API) first, then camelCase (Web API)
        info.update(traverse_obj(author, {
            'channel_follower_count': ('follower_count', {int_or_none}),
            'channel_is_verified': ('custom_verify', {lambda x: bool(x)}),
            'following_count': ('following_count', {int_or_none}),
            'total_favorited': ('total_favorited', {int_or_none}),
            'aweme_count': ('aweme_count', {int_or_none}),
        }) or {})
        info.update(traverse_obj(author, {
            'channel_follower_count': ('followerCount', {int_or_none}),
            'following_count': ('followingCount', {int_or_none}),
            'total_favorited': ('heartCount', {int_or_none}),
            'aweme_count': ('videoCount', {int_or_none}),
        }) or {})

        # Override with explicit stats data when available (user page stats/statsV2)
        if stats_data:
            info.update(traverse_obj(stats_data, {
                'channel_follower_count': ('followerCount', {int_or_none}),
                'following_count': ('followingCount', {int_or_none}),
                'total_favorited': ('heartCount', {int_or_none}),
                'aweme_count': ('videoCount', {int_or_none}),
    ...
    def _parse_aweme_video_app(self, aweme_detail):
        ...
        author_info = self._extract_author_info(aweme_detail)
...
class TikTokUserIE(TikTokBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if re.fullmatch(r'MS4wLjABAAAA[\w-]{64}', user_name):
            ...
            user_detail = detail.get('userInfo', {})
            video_count = traverse_obj(user_detail, (('stats', 'statsV2'), 'videoCount', {int}, any))
            ...
            sec_uid = traverse_obj(user_detail, ('user', 'secUid', {str}))
            if sec_uid:
                fail_early = not traverse_obj(user_detail, ('itemList', ...))
            else:
        ...
        # Extract user profile information from the webpage data
        stats = user_detail.get('stats', {})
        stats_v2 = user_detail.get('statsV2', {})
        playlist_metadata = self._extract_author_info(
            user_detail.get('user', {}), stats_v2 or stats)
        playlist_metadata['channel_id'] = sec_uid
